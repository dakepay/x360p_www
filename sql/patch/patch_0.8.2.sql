...
ADD COLUMN `expect_trial_times` varchar(255) DEFAULT '' COMMENT '期望试听时间' AFTER `next_follow_time`
;

-- 学员自定义字段
ALTER TABLE `x360p_student`
ADD COLUMN `profile` text DEFAULT '' COMMENT '自定义字段JSON保存' AFTER `student_lesson_remain_times`
;
-- 员工自定义字段
ALTER TABLE `x360p_employee`
ADD COLUMN `profile` text DEFAULT '' COMMENT '自定义字段JSON保存' AFTER `user_status`
;
