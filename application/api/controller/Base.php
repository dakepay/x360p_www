...
class Base extends Api
{
    ...
    protected function check_visitor(){
        $request = request();
        $request_method = strtolower($request->method());
        $user = gvar('user');
        if($request_method != 'get' && !empty($user) && isset($user['visitor']) && $user['visitor'] == 1 ){
            return false;
        }
        return true;
    }
    ...
    {
        $visitor_check_result = $this->check_visitor();
        if(!$visitor_check_result){
            return $this->sendError(401,'无操作权限!');
        }
    ...
    public function restful(Request $request)
    {
        ...
        try {
            ...
            if(config('app_debug')){
                ...
                //$response = $this->sendError(500,$e->getMessage(),500);
            }else{
                $response = $this->sendError(500,'server error',500);
            }
        }
    ...
    protected function get_model($name = null){
        ...
        try{
            ...
            if(!in_array($res,$tables)){
                return $this->sendError('404','resource_not_found!' . 'res:'.$res);
            }
            $model = new BaseModel();
            $model->name($res);
        }
    ...
    public function sendSuccess($data = [], $message = 'success', $code = 200, $headers = [], $options = [])
    {
        ...
        if($user && $client && !empty($client['info'])){
            $cache_key = get_today_task_key($client['info']['cid'],$og_id);
            if(!cache($cache_key)){
                $responseData['tts'] = 1;       //tts = today task
            }
        }
     ...
     * @return [type]       [description]
     */
    public function sendConfirm($text = '',$option = [],$headers = []){
        $responseData['error'] = 1;

        $responseData['confirm'] = (string)$text;

        if(!empty($option)) {
            $responseData['option'] = $option;
        }
     ...
     * @return [type] [description]
     */
    protected function getGlobalPers($per_type,$section = ''){
        //$all_tree_pers = config('org_per_item.main');
        $all_tree_pers = get_client_pers_item($per_type,$section);
        ...
        $pers = [];
        foreach($all_tree_pers as $node){
            if(isset($node['need_user_field'])){
                if(!$this->match_user_field($node['need_user_field'],$user)){
                    continue;
                }
            }
            if(isset($node['need_client_field'])){
                if(!$this->match_client_field($node['need_client_field'])){
                    continue;
                }
            }
            if(isset($node['sub'])){
                $node['sub'] = $this->filter_per_tree_node($node['sub'],$user);
                if(empty($node['sub'])){
                    unset($node['sub']);
                }else{
                    if($node['sub'][0]['text'] == 'list' && $node['sub'][0]['uri'] == 'list'){
                        $node['sub'][0]['text'] = $node['text'].'查看';
                        $node['sub'][0]['uri'] = $node['uri'];
                        $node['sub'][0]['ismenu'] = false;
                    }
                }
            }

            $node['text'] = nav_translate($node['text']);

            array_push($pers,$node);
        }
        return $pers;
    }


    private function match_user_field($fields,$user){
        $match = true;
        foreach($fields as $f=>$v){
            if(isset($user[$f]) && $user[$f] == $v){
                continue;
            }else{
                $match = false;
                break;
    ...
    private function match_client_field($fields){
        $client = gvar('client');
        $client_info = $client['info'];
        $match = true;
        foreach($fields as $f=>$v){
            if(isset($client_info[$f]) && $client_info[$f] == $v){
                continue;
            }else{
                $match = false;
                break;
    ...
    private function filter_per_tree_node($nodes,$user){
        $pers = [];
        $has_sub_nav = false;
        foreach($nodes as $node){
            ...
            if(!isset($node['ismenu']) || $node['ismenu'] === true){
                $has_sub_nav = true;
            }
            $node['text'] = nav_translate($node['text']);
            array_push($pers,$node);
        ...
        if(!$has_sub_nav){
            array_unshift($pers,['text'=>'list','uri'=>'list']);
        }
        return $pers;
    }
        ...
        if(!empty($name) && $name != 'all'){
            ...
            switch($name){
                case 'configs':
                    ...
                    break;
                case 'bconfigs':
                    $ret = \app\api\model\Config::BranchConfigs();
                    break;
                case 'pers':
//                    $temp = Config::readConfig('org_per_item');
//                    $ret = $temp['main'];
                    $ret = $this->getGlobalPers('org','main');
                    break;
                case 'mobile_pers':
//                    $temp = Config::readConfig('mobile_org_per_item');
//                    $ret = $temp['main'];
                    $ret = config('mobile_org_per_item.main');
                    break;
                case 'res_pers':
                    $ret = $this->getGlobalPers('org_res');
                    break;
                case 'areas':
                    $ret = db('area', config('center_database'))->select();
                    break;
                case 'branchs':
                    $ret = model('branch')->order('sort', 'asc')->select();
                    break;
                case 'lessons':
                    $ret = model('lesson')->with('lessonMaterial')->select();
                    break;
                case 'departments':
                    $ret = m('department')->where($w_og)->select();
                    break;
                case 'subjects':
                    $ret = model('subject')->with('subjectGrade')->select();
                    break;
                case 'employees':
                    $ret    = [];
                    $append = ['dpt_ids', 'openid'];
                    $employees = model('employee')->select();
                    if($employees){
                        foreach($employees as $k=>$e){
                            $employees[$k] = $e->append($append);
                        }
                        $ret = $employees;
                    }
                    ...
                    break;
                case 'classrooms':
                    $ret = model('classroom')->select();
		             break;
                case 'roles':
                    $ret = model('role')->where($w_og)->whereOr('rid','lt',11)->select();
                    break;
                case 'dicts':
                    $ret = $this->getDictLists();
                    break;
                case 'time_sections':
                    $ret = model('time_section')->select();
                    break;
                case 'season_dates':
                    $ret = model('season_date')->select();
                    break;
                case 'print_tpls':
                    $ret = model('print_tpl')->getAllPrintTplConfig();
                    break;
                case 'holidays':
                    $ret = model('holiday')->select();
                    break;
                case 'accounts':
                    $ret = model('accounting_account')->select();
                    break;
                case 'tally_types':
                    $ret = model('tally_type')->select();
                    break;
                case 'tally_helps':
                    $ret = model('tally_help')->select();
                    break;
		        case 'public_schools':
                    $ret = model('public_school')->select();
                    break;
                case 'public_accounts':
                    $ret = model('Wxmp')->getPublicAccounts();
                    break;
                case 'wechat_template':
                    $ret = config('tplmsg');
                    break;
                case 'review_tpl_setting':
                    $ret = model('ReviewTplSetting')->select();
                    if(empty($ret)) {
                        $ret = [
                ...
                case 'review_styles':
                    ...
                    break;
                case 'config_pay';
                    $ret = model('ConfigPay')->where($w_og)->select();
                    break;
                case 'sms_clound':
                    $ret = user_config('sms_clound');
                    break;
                case 'sms_tpls':
                    $ret = get_sms_tpls();
                    break;
                ...
                case 'navs':
                    ...
                    if(!isset($login_info['client_type'])){
                        $login_info['client_type'] = '';
                    }
                    $ret = User::getLoginUserNavs($login_info,$login_info['client_type']);
                    break;
                case 'quick_navs':
                    $ui_config = get_ui_config('pc');
                    $ret = $ui_config['quick_menu'];
                    break;
                case 'system_links':
                    $ret = config('system_links');
                    break;
                case 'apps':
                    $ret = get_client_apps();
                    break;
            ...
            $global_vars = ['configs','bconfigs','departments',
            'pers','res_pers','roles','branchs','lessons',
            'subjects','classrooms','dicts',
            'time_sections','season_dates','print_tpls',
            'holidays',
            'accounts','tally_types','tally_helps','public_schools','public_accounts','wechat_template',
            'review_tpl_setting','review_tpl_define','config_pay','sms_clound','sms_tpls','words','navs','quick_navs','system_links',
            'apps','review_styles'
            ];
     ...
     * @return [type]       [description]
     */
    protected function getGlobalVarsOfMobile($name = ''){
        $ret = [];
        ...
        if(!empty($name) && $name != 'all'){
            ...
            switch($name){
                case 'configs':
                    ...
                    break;
                case 'mobile_pers':
                    $ret = get_client_pers_item('mobile_org','main');
                    break;
                case 'lessons':
                    $ret = model('lesson')->with('lessonMaterial')->select();
                    break;
                case 'subjects':
                    ...
                    break;
                case 'employees':
                    $ret = model('employee')->field('eid,ename,uid,photo_url')->select();
                    break;
                case 'classrooms':
                    $ret = model('classroom')->select();
                    break;
            ...
            $global_vars = ['configs','mobile_pers','lessons','subjects','employees','classrooms','branchs',
                'review_tpl_setting','review_tpl_define','dicts','roles','accounts','public_schools','tally_types',
                'time_sections','season_dates'];
            foreach($global_vars as $var){
                $ret[$var] = $this->getGlobalVars($var);
            }
        }
    ...
    public function getDictLists(){
        ...
        $m_dict = model('dictionary');
        $top_dicts   = $m_dict->where($w)->field($dict_fields)->order('sort DESC')->skipOgId()->select();
        ...
        if($og_id > 0) {
            //$w['pid'] = ['NEQ',0];
            //$w['og_id'] = ['in', [0, gvar('hog_id')]];
            //$
            $w = "pid != 0 and ((is_system = 1 and og_id = 0) or (is_system != 1 and og_id = {$og_id}) and display = 1)";
        ...
        $dict_items = $m_dict->where($w)->order('sort DESC')->field($dict_fields)->skipOgid(true)->select();

        foreach($dict_items as $item){
    	    if(isset($dict_id_map[$item['pid']])){
                    if($item['is_system'] != 1 && $item['og_id'] != $og_id){
                        continue;
                    }

                    if($item['is_system'] == 1 && $og_id > 0)  {
                        if(!empty($org_system_dictionary[$item['did']]) && $org_system_dictionary[$item['did']]['display'] == 1) {
                            $item = $org_system_dictionary[$item['did']];
                        } else {
                            continue;
                        }
                    }
