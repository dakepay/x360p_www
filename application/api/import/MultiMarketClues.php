...
class MultiMarketClues extends Import{
    ...
    protected $fields = [
        ...
        ['field'=>'mc_id','name'=>'渠道名称','must'=>true],
    ...
    protected function init_did_map()
    {
        ...
        $from_dids = $mDictionary->field('did,name')->where($w)->select();
    ...
    {
    	$w['channel_name'] = trim($value);
    	$w['og_id']        = gvar('og_id');
    	$w['bid']          = request()->bid;
        $mc_id = model('MarketChannel')->where($w)->find();

        if(empty($mc_id)) exception('来源渠道不存在!');
    ...
    protected function convert_family_rel($value)
    {
