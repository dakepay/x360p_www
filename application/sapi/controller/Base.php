...
class Base extends Api
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        $sid = $request->header('x-sid');
        $sid = $sid ? $sid : 0;
        $request->bind('sid', $sid);
...
//        ob_end_clean();
    ...
    protected function validate($data, $validate, $message = [], $batch = false, $callback = null)
    {
        if (is_array($validate)) {
            ...
            try{
                ...
                $class = 'app\\sapi\\validate\\'.Loader::parseName($validate,1);
        ...
        if(!empty($name) && $name != 'all'){
            ...
            switch($name){
                ...
                case 'dicts':
                    ...
                    $ret = (new \app\api\controller\Base())->getDictLists();
                    break;
                case 'public_schools':
    ...
    protected function getDictLists(){
        ...
        foreach($dict_items as $item){
    	    if(isset($dict_id_map[$item['pid']])){
    	            ...
    	            $sub_items = $this->get_sub_dict_lists($item['did'],$dict_items);
    	            if(!empty($sub_items)){
    	                foreach($sub_items as $si){
    	                    array_push($dicts[$name],$si);
                        }
                    }
    	    }
        }
    ...
    private function get_sub_dict_lists($did,$dict_items){
        $ret = [];
        foreach($dict_items as $item){
            if($item['pid'] == $did){
                array_push($ret,$item);
