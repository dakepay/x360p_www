...
use think\Response;
use think\Exception;
use app\sapi\model\Base as BaseModel;
...
class Base extends Api
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        if(config('app_debug')) {
            ...
            if($developer == 'luo') $this->apiAuth = false;

            $luo_param = $request->param('luo');
            if(isset($luo_param)) $this->apiAuth = false;
        }
    ...
    protected function validate($data, $validate, $message = [], $batch = false, $callback = null)
    {
        if (is_array($validate)) {
            ...
            try{
                ...
                $class = 'app\\ftapi\\validate\\'.Loader::parseName($validate,1);
        ...
        if(!empty($name) && $name != 'all'){
            ...
            switch($name){
                case 'configs':
                    ...
                    break;
                case 'mobile_pers':
                    $ret = get_client_pers_item('mobile_student','main');
                    break;
                case 'lessons':
                    $ret = model('lesson')->with('lessonMaterial')->select();
                    break;
                case 'subjects':
                    $ret = model('subject')->select();
                    break;
                case 'classrooms':
            ...
            $global_vars = ['configs','lessons','subjects','classrooms','dicts','review_tpl_setting','review_tpl_define','employees','branchs'];
