...
class Base extends Api
{
        ...
        if(!empty($name) && $name != 'all'){
            ...
            switch($name){
                case 'areas':
                    ...
                    break;
                case 'pers':
                    $ret = $this->getGlobalPers('fms','main');
                    break;
                case 'roles':
                    $w['rid']   = ['BETWEEN',100,200];
                    $w['og_id'] = 0;
                    $ret = db('role')->where($w)->select();
                    break;
                case 'dicts':
                ...
                case 'navs':
                    ...
                    $ret = User::getLoginUserNavs($login_info,$login_info['client_type'],'fms');
                    break;
                default:
                    break;
            }
    ...
    protected function getDictLists(){
        ...
        $top_dicts   = db('dictionary')->where(['pid'=>0])->field($dict_fields)->order('sort DESC')->select();
        $dict_id_map = [];
        $dicts = [];
        ...
        $dict_items = db('dictionary')->where('pid','NEQ',0)->where('display',1)->order('sort DESC')->field($dict_fields)->select();
